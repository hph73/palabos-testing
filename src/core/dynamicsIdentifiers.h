...
void createIdIndirection(
    std::map<int, std::string> const &foreignIdToName, std::map<int, int> &idIndirect);

template <typename T, template <typename U> class Descriptor>
struct DynamicsGenerator {
    virtual ~DynamicsGenerator() { }
    virtual Dynamics<T, Descriptor> *generate(HierarchicUnserializer &unserializer) const = 0;
};
...
class TwoParamDynamicsGenerator : public DynamicsGenerator<T, Descriptor> {
    virtual Dynamics<T, Descriptor> *generate(HierarchicUnserializer &unserializer) const
    {
        return new TwoParamDynamics(unserializer.readValue<T>(), unserializer.readValue<T>());
    }
};
...
class CompositeDynamicsGenerator : public DynamicsGenerator<T, Descriptor> {
    virtual Dynamics<T, Descriptor> *generate(HierarchicUnserializer &unserializer) const
    {
        ...
        Dynamics<T, Descriptor> *compositeDynamics =
            dynamicsRegistration<T, Descriptor>().generate(unserializer);
